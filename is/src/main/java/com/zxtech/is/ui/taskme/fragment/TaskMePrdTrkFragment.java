...
public class TaskMePrdTrkFragment extends BaseFragment implements BGARefreshLayout.BGARefreshLayoutDelegate, BaseQuickAdapter.OnItemClickListener {
    ...
    public static TaskMePrdTrkFragment newInstance() {
        return new TaskMePrdTrkFragment();
    }
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        if (taskDefKey.equals("s2_fe")) {
            Intent intent = new Intent(mContext, S2TaskListActivity.class);
            intent.putExtra("projectId", m.get("prjId").toString());
            intent.putExtra("procDefKey", "s2");
            intent.putExtra("taskId", m.get("id").toString());
            startActivityForResult(intent, REQUEST);
        } else {
            Intent intent = new Intent(mContext, S2TaskReviewActivity.class);
            intent.putExtra("projectId", m.get("prjId").toString());
            intent.putExtra("procDefKey", "s2");
            intent.putExtra("taskId", m.get("id").toString());
            startActivityForResult(intent, REQUEST);
    ...
    private void requestNet() {
        ...
        taskMeService.taskMeCommon("s2", pageNum, pageSize)
