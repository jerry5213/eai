...
import com.zxtech.is.ui.task.adpter.S1ElevatorReviewAdpter;
...
public class S1TaskReviewActivity extends BaseActivity implements BaseQuickAdapter.OnItemClickListener {
    ...
    private S1ElevatorReviewAdpter s1ElevatorReviewAdpter;
    //产品列表
    ...
    protected int getLayoutId() {
        return R.layout.activity_task_s1_review;
    }
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        s1ElevatorReviewAdpter = new S1ElevatorReviewAdpter(R.layout.item_s1_elevator_review, elevatorList);
        rv_is_elevator.setAdapter(s1ElevatorReviewAdpter);

        //加载数据
        loadData(params);

        projectAttachReviewAdpter.setOnItemClickListener(this);
        s1ElevatorReviewAdpter.setOnItemClickListener(this);
    }
    ...
    public void loadData(Map<String, String> params){
        ...
        s1Service.selectISPlanS1(params).compose(RxHelper.<BaseResponse<S1Response>>rxSchedulerHelper()).subscribe(new DefaultObserver<BaseResponse<S1Response>>(getActivity(),true) {
            ...
            public void onSuccess(BaseResponse<S1Response> response) {
                ...
                s1ElevatorReviewAdpter.notifyDataSetChanged();
            }
        });
    }
    ...
    private void submitPlanS1(String checkResult) {
        int count = s1ElevatorReviewAdpter.getSelectCount();
        if (count == 0) {
            ...
            for (int i = 0; i < elevatorList.size(); i++) {
                S1Elevator s1Elevator = elevatorList.get(i);
                if (s1Elevator.isCheck()) {
                    review = new ReviewEntity();
                    review.setProcinstid(s1Elevator.getProcinstid());
                    review.setTaskId(s1Elevator.getTaskId());
            ...
            S1Service s1Service = HttpFactory.getService(S1Service.class);
            s1Service.submitPlanS1(requestBody).compose(RxHelper.<BaseResponse<Boolean>>rxSchedulerHelper()).subscribe(new DefaultObserver<BaseResponse<Boolean>>(getActivity(), true) {
                @Override
...
//            if("".equals(comment)){
//                ToastUtil.showLong(getResources().getString(R.string.is_check_conment_null_msg));
//            }else {
//
//            }
        }
    ...
    private void clickSelectAll() {
        if (cb_is_checkall.isChecked()) {
            s1ElevatorReviewAdpter.setCheckAllFlag(true);
        } else {
            s1ElevatorReviewAdpter.setCheckAllFlag(false);
        }
        for (int i = 0; i < elevatorList.size(); i++) {
            if (s1ElevatorReviewAdpter.isCheckAllFlag()) {
...
//                }
        ...
        if (adapter instanceof S1ElevatorReviewAdpter) {
            Intent intent = new Intent(mContext, S1ElevatorDetailReviewActivity.class);
