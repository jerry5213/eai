...
import com.zxtech.is.ui.task.adpter.S2ElevatorReviewAdpter;
...
public class S2TaskReviewActivity extends BaseActivity implements BaseQuickAdapter.OnItemClickListener {
    ...
    private S2ElevatorReviewAdpter s2ElevatorReviewAdpter;
    //产品列表
    ...
    protected int getLayoutId() {
        return R.layout.activity_task_s2_review;
    }
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        s2ElevatorReviewAdpter = new S2ElevatorReviewAdpter(R.layout.item_s2_elevator_review, elevatorList);
        rv_is_elevator.setAdapter(s2ElevatorReviewAdpter);

        //加载数据
        loadData(params);

        projectAttachReviewAdpter.setOnItemClickListener(this);
        s2ElevatorReviewAdpter.setOnItemClickListener(this);
    }
    ...
    public void loadData(Map<String, String> params){
        ...
        s2Service.selectISPlanS2(params).compose(RxHelper.<BaseResponse<S2Response>>rxSchedulerHelper()).subscribe(new DefaultObserver<BaseResponse<S2Response>>(getActivity(),true) {
            @Override
            public void onSuccess(BaseResponse<S2Response> response) {
                //产品信息
                elevatorList.clear();
                elevatorList.addAll(response.getData().getElevatorList());
                s2ElevatorReviewAdpter.notifyDataSetChanged();
            }
        });
    }
    ...
    private void submitPlanS2(String checkResult) {
        int count = s2ElevatorReviewAdpter.getSelectCount();
        if (count == 0) {
            ...
            for (int i = 0; i < elevatorList.size(); i++) {
                S2Elevator s2Elevator = elevatorList.get(i);
                if (s2Elevator.isCheck()) {
                    review = new ReviewEntity();
                    review.setProcinstid(s2Elevator.getProcinstid());
                    review.setTaskId(s2Elevator.getTaskId());
            ...
            S2Service s2Service = HttpFactory.getService(S2Service.class);
            s2Service.submitPlanS2(requestBody).compose(RxHelper.<BaseResponse<Boolean>>rxSchedulerHelper()).subscribe(new DefaultObserver<BaseResponse<Boolean>>(getActivity(), true) {
                @Override
    ...
    private void clickSelectAll() {
        if (cb_is_checkall.isChecked()) {
            s2ElevatorReviewAdpter.setCheckAllFlag(true);
        } else {
            s2ElevatorReviewAdpter.setCheckAllFlag(false);
        }
        for (int i = 0; i < elevatorList.size(); i++) {
            if (s2ElevatorReviewAdpter.isCheckAllFlag()) {
...
//                }
        ...
        if (adapter instanceof S2ElevatorReviewAdpter) {
            Intent intent = new Intent(mContext, S2ElevatorDetailReviewActivity.class);
