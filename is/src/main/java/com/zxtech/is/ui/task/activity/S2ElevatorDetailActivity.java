...
import com.zxtech.is.model.s2.S2Request;
...
public class S2ElevatorDetailActivity extends ElevatorDetailCommonActivity {
    //预计3C日期
    ...
    protected int getLayoutId() {
        return R.layout.activity_task_s2_elevator_detail;
    }

    public void loadData() {
        S2Elevator elevator = (S2Elevator) getIntent().getSerializableExtra("elevator");
        ...
        //预计3C日期
        String plan3CDate = elevator.getPlan3cdate() == null ? elevator.getS1Plan3CDate() : elevator.getPlan3cdate();
        String moneyS2Date = elevator.getMoneys2date() == null ? elevator.getS1MoneyS2Date() : elevator.getMoneys2date();
        String moeny3CDate = elevator.getMoeny3cdate() == null ? elevator.getS1Moeny3CDate() : elevator.getMoeny3cdate();
    ...
    protected void save() {
        ...
        if (checkData()) {
            ...
            S2Request s2Request = packData();
            String param = GsonUtils.toJson(s2Request, false);
            RequestBody requestBody = RequestBody.create(MediaType.parse("text/plain"), param);
            S2Service s2Service = HttpFactory.getService(S2Service.class);
            s2Service.savePlanS2(requestBody, files).compose(RxHelper.<BaseResponse<Boolean>>rxSchedulerHelper()).subscribe(new DefaultObserver<BaseResponse<Boolean>>(getActivity(), true) {
                @Override
    ...
    protected S2Request packData() {
        ...
        S2Elevator s2Elevator = new S2Elevator();
        s2Elevator.setElevatorguid(elevatorGuid);
        s2Elevator.setProcinstid(procInstId);
        s2Elevator.setPlan3cdate(plan3CDate);
        s2Elevator.setMoneys2date(moneyS2Date);
        s2Elevator.setMoeny3cdate(moeny3CDate);
        s2Elevator.setWellholeitem(wellholeItem);
        s2Elevator.setEnteritem(enterItem);
        s2Elevator.setYarditem(yardItem);
        s2Elevator.setInstalltemitem(installTemItem);
        s2Elevator.setStorehouseitem(storehouseItem);
        s2Elevator.setConstructionitem(constructionItem);
        s2Elevator.setProjectAddProvince(province);
        s2Elevator.setProjectAddCity(city);
        s2Elevator.setProjectAddArea(area);
        s2Elevator.setProjectAddOther(otherAddress);
        s2Elevator.setInstlProjectAddProvince(installProvince);
        s2Elevator.setInstlProjectAddCity(installCity);
        s2Elevator.setInstlProjectAddArea(installArea);
        s2Elevator.setInstlProjectAddOther(installOtherAddress);
        s2Elevator.setContactsList(contactList);

        //删除的附件
        s2Elevator.setDeleteFileList(deleteFileList);

        S2Request s2Request = new S2Request();
        s2Request.setS2Elevator(s2Elevator);
        s2Request.setProjectGuid(projectGuid);
        s2Request.setTaskId(taskId);
        s2Request.setProcDefKey(procDefKey);
        return s2Request;
    }
